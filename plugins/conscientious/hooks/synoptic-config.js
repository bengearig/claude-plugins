#!/usr/bin/env node
// conscientious / synoptic — shared flag-file I/O for the commit-message feature.
//
...
//
// The flag lives at $CLAUDE_CONFIG_DIR/.synoptic-active (default
// ~/.claude/.synoptic-active). Valid contents: "on", "auto", or "off".
// Missing/invalid is treated as OFF by readers.
//
// Synoptic keeps the three-state shape of clarify and biblio but swaps the roles
// of the middle and off states: OFF injects nothing (the neutral state the older
// toggles spell AUTO), while AUTO injects a softer directive that yields to a
// repo's own documented commit convention. There is no "commit verbosely"
// directive to hang off OFF, so OFF is the silent one.
...
function getFlagPath() {
    ...
    return path.join(claudeDir, '.synoptic-active');
}
...
function safeWriteFlag(content) {
    ...
    try {
        ...
        const tempPath = path.join(realFlagDir, `.synoptic-active.${process.pid}.${Date.now()}`);
...
// Returns 'on' / 'auto' / 'off' on success, null on any anomaly (missing,
// symlink, oversized, unknown value). Callers default to OFF when null.
function readFlag() {
    const flagPath = getFlagPath();
    try {
        let st;
