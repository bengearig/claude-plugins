...
// single setup message pointing at the combined conscientious-statusline.sh
// script so Claude can proactively offer to wire it up. Detection is "any
// statusLine present", so users who already have any statusline (including the
// legacy clarify-only one) won't be re-nudged.
...
try {
    ...
    if (!hasStatusline) {
        const scriptPath = path.join(__dirname, 'conscientious-statusline.sh');
