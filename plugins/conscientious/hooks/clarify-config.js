#!/usr/bin/env node
// conscientious / clarify — shared flag-file I/O for the clarify feature.
//
// Adapted from caveman's caveman-config.js:
//   - safeWriteFlag: symlink-safe, atomic temp+rename, 0600 perms, parent-dir
//     ownership check, O_NOFOLLOW where available.
//   - readFlag: symlink-safe, size-capped, value whitelist. Returns null on
//     any anomaly so callers never act on attacker-planted bytes.
//
// The flag lives at $CLAUDE_CONFIG_DIR/.clarify-active (default ~/.claude/.clarify-active).
// Valid contents: "on", "auto", or "off". Missing/invalid is treated as ON by readers.
...
function getFlagPath() {
    ...
    return path.join(claudeDir, '.clarify-active');
}
...
function safeWriteFlag(content) {
    ...
    try {
        ...
        const tempPath = path.join(realFlagDir, `.clarify-active.${process.pid}.${Date.now()}`);
...
// oversized, unknown value). Callers default to ON when null.
function readFlag() {
    const flagPath = getFlagPath();
    try {
        let st;
