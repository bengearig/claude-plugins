#!/usr/bin/env node
// conscientious / taciturn — shared flag-file I/O for the comment-brevity feature.
//
...
//
// The flag lives at $CLAUDE_CONFIG_DIR/.taciturn-active (default
// ~/.claude/.taciturn-active). Valid contents: "on" or "off".
// Missing/invalid is treated as ON by readers.
//
// Unlike the sibling toggles, taciturn has no AUTO state: "no directive" and
// "comment normally" are the same behavior, so the middle state bought nothing.
// A pre-release "auto" left on disk therefore fails the whitelist below and
// readers fall back to ON; taciturn-activate.js rewrites the file on next
// SessionStart.
...
function getFlagPath() {
    ...
    return path.join(claudeDir, '.taciturn-active');
}
...
function safeWriteFlag(content) {
    ...
    try {
        ...
        const tempPath = path.join(realFlagDir, `.taciturn-active.${process.pid}.${Date.now()}`);
...
// oversized, unknown value — including a pre-release 'auto'). Callers default
// to ON when null.
function readFlag() {
    const flagPath = getFlagPath();
    try {
